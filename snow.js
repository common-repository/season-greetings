...
window.onload=function(){
	...
	for (i = 0; i < nks.snowflakes; i++) {
		if (ieversion && ieversion==6) {
			// ie sucks
			document.getElementById('seasongreets' + i).style.position='absolute';
		}
	}
	snow(nks);
	if (nks.oldonload) {
...
function snow(nks) {
	...
	for (i = 0; i < nks.snowflakes; i++) {
		if (nks.active[i] === true) {
			...
			document.getElementById('seasongreets' + i).style.top=nks.posy[i] + "px";
			document.getElementById('seasongreets' + i).style.left=nks.posx[i] + "px";
		}
	}
